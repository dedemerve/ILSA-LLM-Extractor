"""Extract first 5 PDFs from ~/Desktop/articles/ and save JSON outputs."""
...
def main():
    ...
    pdfs = sorted(ARTICLES_DIR.glob("*.pdf"))[:5]
    if not pdfs:
        print("No PDFs found in ~/Desktop/articles/")
        return
    ...
    for i, pdf_path in enumerate(pdfs, 1):
        ...
        print(f"  Not-reported explanation: {(hnre or '(null)')[:120]}")
        ...
        print(f"  Countries ({len(countries)}): "
              f"{[(c['country_code'], c['n_students']) for c in countries[:5]]}")

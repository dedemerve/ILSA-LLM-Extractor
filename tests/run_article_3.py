"""Re-extract article 3 (Zhou et al.) with anti-N/A confounder rules."""
...
def main():
    ...
    pdf_path = next(p for p in ARTICLES_DIR.glob("*.pdf") if p.name.startswith("3."))
    ...
    for c in conf:
        print(f"  [{c['category']:18s}] {c['variable_code']:25s} → {c['variable_name']}")
    print(f"\nSaved: {out_path}")
